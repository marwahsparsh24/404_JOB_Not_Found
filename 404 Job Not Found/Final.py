...
def save_uploaded_file(filedata):
    if filedata:
...
# === Core Function ===
def upload_rejected_resume_local_to_df(resume_filename, job_description_text, application_status):
    ...
    jd_row = {
        ...
        "Plain Text Resume": job_description_text,
        "Metadata": {
            "application_status": application_status,
            "resume_file": resume_filename,
            "type": "job_description"
...
def run_formatting_compliance_last(df):
    if df.empty:
...
2. Use readable fonts. Top ATS friendly fonts are: Times New Roman, Tahoma, Verdana, Arial, Helvetica, Calibri, Georgia, 
Cambria, Gill Sans, Garamond. if the font in the resume is close to any of these then it is obeys this rule.
...
def ats_parsing_error_detection_agent(df):
    if df.empty:
        return "⚠️ DataFrame is empty."

    non_jd_df = df[~df["Resume File"].str.startswith("jd_")]
    if non_jd_df.empty:
...
def get_job_recommendations(resume_text):
    ...
    base_query = " ".join(query_keywords)
    ...
    def fetch_jobs(query):
        ...
        for page in range(1, 3):
            ...
            response = requests.get(
                ...
                headers={
                    "X-RapidAPI-Key": "f4fbcdea11msh162ecb022aac4b2p166660jsn0e187622228b",
...
def get_formatted_df():
    """Returns the global formatted_resume_df"""
    global formatted_resume_df
    return formatted_resume_df

def get_plain_df():
    """Returns the global plain_resumes_df"""
    global plain_resumes_df
    return plain_resumes_df
